...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Function to Insert Questions Without Repetition
    function insertQuestions($conn, $table, $type, $count, $exam_id) {
        // Get unique questions that are not yet used in this exam
        $query = "
            SELECT id FROM $table 
            WHERE id NOT IN (SELECT question_id FROM exam_questions WHERE exam_id = ?) 
            AND question_type = ? 
            ORDER BY RAND() LIMIT ?";
        
        $stmt = $conn->prepare($query);
        $stmt->bind_param("isi", $exam_id, $type, $count);
        $stmt->execute();
        $result = $stmt->get_result();
